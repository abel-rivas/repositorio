...
function meta_news_author_link() {
	return '<div class="author-link">' .
		sprintf(
			esc_html__('INRH  -  %s', 'meta-news'),
			'<a href="' . esc_url('https://localhost/intranet') . '" target="_blank" rel="noopener noreferrer" title="' . esc_attr__('TICA', 'meta-news') . '" >' . esc_html__( gettext('TICA Information Technology Communications and Automation'), 'meta-news') . '</a>'
		) . '</div>';
}
